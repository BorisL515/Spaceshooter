...
def main():
    ...
    def redraw_window():
        ...
        if lost:
            ...
            WIN.blit(lost_label, (WIDTH/2 - level_label.get_width()/1.6, HEIGHT/2 - level_label.get_height()/2))
...
def main_menu():
    title_font = pygame.font.SysFont("comicsans", 44)
    run=True
    while run:
        WIN.blit(BG, (0,0))
        title_label = title_font.render("Press any mouse button to begin.....", 1, (255,255,255))
        WIN.blit(title_label, (WIDTH/2 - title_label.get_width()/2, 345))
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
